...
`tests/run.sh` is 835 offline assertions with no network, no model and no pull request. It stubs `gh` and `claude` onto PATH and builds throwaway git repositories with real histories and real bare origins, so the assertions are about what CrossRev actually did rather than what it printed.
