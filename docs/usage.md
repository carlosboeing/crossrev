...
A pull request that changes no files never converges either. Its head is identical to its base — the branch was reverted, or the same work reached the base by a merge, a rebase or a cherry-pick — so there is no diff to read and no required file to account for. CrossRev settles that pass itself, without calling a reviewer: it posts a summary naming the state and applies `crossrev/halted`. Close the pull request, or push a commit and the loop starts again.
