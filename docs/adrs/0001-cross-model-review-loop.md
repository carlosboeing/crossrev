...
- **The credential boundary is two-sided.** Stripping GitHub tokens from the harness process covers what reaches the model. It says nothing about what leaves it: a reviewer quoting a configuration file, a fixture or a `.env` example puts the matched string into a finding, and the orchestrator publishes that text to a public pull request. Every body the orchestrator writes to GitHub therefore passes a credential-shape filter first, at the publish call and not at the parse — filtering earlier would make a leg's payload depend on whether a run directory exists. The shapes and the three filtered outputs are listed in [configuration](../configuration.md#logs).
