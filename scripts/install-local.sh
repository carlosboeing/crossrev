#!/usr/bin/env bash
#
# install-local.sh — build crossrev from this checkout and put it on your PATH.
#
...
# The repository root, not this script's directory: install-local.sh lives in
# scripts/, one level down from the checkout it builds.
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

if [[ ! -f "$HERE/scripts/build-binary.sh" ]]; then
  echo "error  install-local.sh builds the binary with scripts/build-binary.sh, which is missing." >&2
  echo "       Either that is not a CrossRev source tree, or the ref you cloned predates it." >&2
  exit 1
fi

command -v go >/dev/null 2>&1 || {
  echo "error  install-local.sh builds CrossRev from source, and go is not installed." >&2
...
_no_input() {
  echo "error  install-local.sh needs to ask you something, but no terminal is attached." >&2
...
while (( $# )); do
  case "$1" in
    ...
    --help|-h)
      echo "usage: scripts/install-local.sh [--yes] [--bin-dir <dir>] [--skills | --no-skills]"; exit 0 ;;
...
if [[ "$WANT_SKILLS" == "0" ]]; then
  ...
  if [[ "$WANT_SKILLS" == "1" ]] || _confirm "Hand over to the skills CLI now?"; then
    # Interactive unless install-local.sh was itself told not to ask. --yes here means
