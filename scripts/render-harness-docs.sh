...
def notes_str:
  (if .schema_style == "inline" then "Takes the schema **inline** as a JSON string."
   ...
   elif .schema_style == "prompt" then "No schema flag: the schema travels inside the prompt, and CrossRev extracts the JSON from the answer text itself. Review-only"
   else "" end) +
