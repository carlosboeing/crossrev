...
ASSETS=(
  ...
  # under templates/ is listed, because the `go:embed` there is a glob and a
  # file left out would be embedded with nothing checking it.
  ...
  # The version `crossrev version` prints. A binary has no checkout, so
  # internal/cli carries the bytes. Copied rather than rendered, so the
  # trailing newline the file holds survives and internal/cli deletes
  # whitespace the way `tr -d` does.
