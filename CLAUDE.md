...
**Pre-1.0, and honest about how far the proof reaches.** Every command is covered by an offline suite and the local path has run against real pull requests. **Automated mode's workflows are installed in one repository, [`carlosboeing/crossrev-testbed`](https://github.com/carlosboeing/crossrev-testbed), and the loop has chained leg to leg there on GitHub's runners** — at v0.2.0 on 2026-08-17, at v0.5.0 on 2026-08-24, and on the native binary at v0.6.1 on 2026-09-06, where a pull request carrying planted defects ran five legs to `crossrev/converged` unattended. **The two cron workflows were outside every one of those runs**, which is how v0.6.0 shipped both of them broken; they were watched run green at v0.6.2 on 2026-09-16, by `workflow_dispatch` rather than by the schedule. v0.7.0 re-proved the event legs with coverage on a two-defect pull request that ran review, resolve and review again to `crossrev/converged`, with both crons green by dispatch — and caught SHA pins never having pinned the binary; v0.7.1 repairs that, proven by a pre-release pin resolving the older release after latest moved on. v0.7.3 stops a pass that enumerates no reviewable file reporting green, proven on hosted runners by the same fixture converging at v0.7.2 and halting at v0.7.3, and by an ordinary pull request still converging with a published coverage generation. Both crons have now also been watched run green on their own schedule rather than by dispatch. The ref-store change was proven over the local route on 2026-09-21 (candidate `0.7.3-ac64604`, clean tree): testbed pull request #32 ran review, resolve and review again to `crossrev/converged` with zero machine comments and a two-generation chain on its slot ref, under agy reviewing and claude resolving. The proof caught ref updates sending a string force — every update 422d into a silent marker fallback — fixed and covered before the run. It does not re-prove CI event chaining, which this work leaves untouched. v0.8.0 re-proves that chaining on hosted runners the same day: a repin pull request reviewed by its own new pin from the head, a three-defect proof pull request running review, resolve and review again to `crossrev/converged`, the pass's generations on its slot ref, and both crons green by dispatch. v0.9.0 proves the checkout credential hardening the same way on 2026-09-26: both crons green by dispatch on a pre-release pin and again at the release pin, a repin pull request converging on pass one under the new binary, and a two-defect proof pull request running review, resolve and review again to `crossrev/converged`, with the resolve push authenticating through the new helper and neither leg's run log carrying a `credentials` event — the leg writes one only when it removes something. A second proof pull request (#37) covers the scrub's removal path: old templates without `persist-credentials: false` at the v0.9.0 pin, every leg logging the removal of the one persisted entry from `.git/config`, and the pass converging.
...
- **The Bash implementation is removed, and Go is where a defect gets fixed.** `bin/crossrev` and `lib/` are gone; every route runs the binary. The frozen oracle under `tests/fixtures/parity/` stays read-only: Go reads those vectors and never writes them.
- **Judge a finding before you file it.** Fix a code defect in this branch. Fix a comment or a typo when it takes minutes, drop it when it does not. **File no GitHub issue.** An issue is for work somebody has to schedule, and the backlog is not a place to record that a sentence read badly.
...
- **Run three checks before claiming done.** `bash tests/run.sh` must report `all suites passed`, `go test ./...` must pass, and `bash scripts/lint.sh` must report `lint clean`. The suite builds the binary once and runs every remaining shell suite against it. All three are offline. `lint.sh` is offline once the pinned `go1.27.0` toolchain has been fetched, and fetches it on first use when the installed Go is a different version. None is an excuse for asserting instead of verifying. `lint.sh` is quick. The suite runs its 9 files in parallel, one job per core up to eight. `-j 1` restores the single-file order when you want to watch one suite.
