...
- **Run both checks before claiming done.** `bash tests/run.sh` must report `all suites passed` and `bash scripts/lint.sh` must report `lint clean`. Both are offline, so there is no excuse for asserting instead of verifying. `lint.sh` is quick; the suite is not — start it before you need the answer rather than at the end.
