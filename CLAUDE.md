...
- **Run both checks before claiming done.** `bash tests/run.sh` must report `all suites passed` and `bash scripts/lint.sh` must report `lint clean`. Both are offline, so there is no excuse for asserting instead of verifying. `lint.sh` is quick. The suite runs its 28 files in parallel, one job per core up to eight, which measured about 90 seconds against about 380 seconds one at a time. `-j 1` restores the single-file order when you want to watch one suite.
