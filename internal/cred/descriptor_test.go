...
func TestEmbeddedDescriptorIsTheCanonicalFile(t *testing.T) {
	canonical, err := os.ReadFile("../../assets/harnesses.json")
	if err != nil {
		t.Fatalf("reading assets/harnesses.json: %v", err)
	}
	if string(cred.DescriptorJSON()) != string(canonical) {
		t.Error("the embedded descriptor differs from assets/harnesses.json — run scripts/sync-embedded-assets.sh")
	}
}
