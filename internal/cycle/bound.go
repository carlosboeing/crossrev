...
func finishAtBound(ctx context.Context, d *Driver, in BoundInput) BoundResult {
	...
	if marker.State != core.PassComplete {
		...
		// and the marker's own state answers it (lib/run.sh:2803-2821).
		//
		...
		// lib/run.sh:2822-2824. The reload names the loaded repository rather
		// than the one the operator typed, which is the one place this argv
		// differs from the loop's (lib/run.sh:2938, :2950, :2974).
		var err error
	...
	// the bound was the reason it stopped (lib/run.sh:2856-2886).
	var err error
	state, err = d.Loader.Load(ctx, reloadLoaded(state))
	if err != nil {
		return BoundResult{Failed: true, Err: err}
...
// reloadLoaded is `ctx_load "$CTX_PR" "$CTX_REPO"` (lib/run.sh:2822, :2860):
...
// readReview is the decision the loop (lib/run.sh:2952-2967) and the bound
// (lib/run.sh:2831-2845) both take off the review marker for the current pass.
//
...
// readResolve is the decision the loop (lib/run.sh:2975-3010) and the bound
// (lib/run.sh:2861-2886) both take after the resolve leg.
//
...
// already at the cap (lib/run.sh:2941) and the bound prints when the pass at
// the cap is already resolved (lib/run.sh:2849).
