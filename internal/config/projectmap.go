...
// at lib/config.sh:388. Written as the same character class rather than as two
...
// order lib/config.sh:380 reads them.
...
// where the loop writes (lib/config.sh:372-402, ADR 0003).
func ProjectMapTracker(ctx context.Context, base core.Revision, show ShowFile) (string, bool, error) {
	for _, path := range projectMapFiles {
		...
		// moves to the next file (lib/config.sh:399-401), so `- **Tracker**:
...
func trackerIn(document string) (string, bool) {
	...
	for _, line := range strings.Split(document, "\n") {
		...
		// sub-heading inside it does not (lib/config.sh:388).
...
// path, a tracker name, or `none` (lib/config.sh:392-398).
func stripGloss(value string) string {
	if at := strings.Index(value, "("); at >= 0 {
		value = value[:at]
	}
	// The Bash sed is `s/[[:space:]]*$//` at lib/config.sh:398, and POSIX
