...
type Spec struct {
	...
	//
	// An allowlist withholds a credential nobody named. The runner instance
	// is what withholds one somebody did: NewOSRunner refuses a forge
	// credential, NewOrchestratorRunner may start a child that holds one.
	Env []string
...
//
// NewOSRunner refuses a Spec that still carries one, which is the guard
