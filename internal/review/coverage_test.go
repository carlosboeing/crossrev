...
// oversizedHeaderBody is one generated-file body no rendered prompt can hold
// alone: the header marker in the first line, then bulk.
func oversizedHeaderBody() string {
	return "// generated by scripts/build-embed.mjs — do not edit\n" + strings.Repeat("a", 200*1024)
}

// A generated file that fits no prompt moves from Required to Excluded
// before the first publication: the initial generation records the skip with
...
func TestReviewMovesSkipsIntoExclusionsBeforePublishing(t *testing.T) {
	...
	wantReason := "generated (header), " + strconv.Itoa(len(big)) + " bytes, over the 184320-byte prompt budget"

	gens := ledgerGenerations(t, e)
	if len(gens) < 2 {
		t.Fatalf("generations = %d, want the initial one plus the accepted batch", len(gens))
	}
	initial := gens[0]
	if len(initial.Excluded) != 1 {
		t.Fatalf("initial exclusions = %v, want the skipped file", initial.Excluded)
	}
	if initial.Excluded[0].Path != "src/webAssets.ts" || initial.Excluded[0].Reason != wantReason {
		t.Errorf("initial exclusion = %+v, want src/webAssets.ts with %q", initial.Excluded[0], wantReason)
	}
	for _, gen := range gens {
		for _, record := range gen.Records {
			...
			if gen.Paths[record.PathIndex] == "src/webAssets.ts" {
				t.Errorf("gen %d carries a unit record for the skipped file", gen.Gen)
			}
		}
	}
	// a.go stays outstanding until its batch is accepted, then covered.
	last := gens[len(gens)-1]
	covered := 0
	for _, record := range last.Records {
		if record.Type == "unit" && record.Verdict.Present() {
	...
	if covered != 1 {
		t.Errorf("covered units in the final generation = %d, want 1", covered)
	}
	if e.runner.calls != 1 {
		t.Errorf("harness calls = %d, want 1 (a.go alone)", e.runner.calls)
	}
}

// The same skip reason lands in the marker store when the ref write is
// refused: whichever store holds the ledger, the skip is recorded.
func TestReviewSkipReasonReachesTheMarkerStore(t *testing.T) {
	e := newEnv(t)
	writeRequiredHead(e, "a.go", "package a\n")
	big := oversizedHeaderBody()
	writeRequiredHead(e, "src/webAssets.ts", big)
	...
	store.SetPublishRefused(&prstate.RefWriteRefused{Op: "updating ref", Err: errors.New("gh exited 1")})
	e.runner.script = []exec.Result{
		{ExitCode: 0, Stdout: claudeStdout(batchAnswer(t, 1))},
	}
	got := runLeg(t, e, e.request(t))
	...
	handle, claimed, err := final.CoverageHandle()
	if err != nil || !claimed {
		t.Fatalf("final marker claims no coverage: claimed=%v err=%v", claimed, err)
	}
	payload, ok := handle.Payload.Get()
	if !ok {
		t.Fatal("the marker-store handle carries no payload")
	}
	if !strings.Contains(string(payload), "generated (header)") || !strings.Contains(string(payload), "src/webAssets.ts") {
		t.Errorf("marker-store payload lacks the skip record: %s", payload)
	}
}

// A re-driven pass at the same base and head recomputes the same skip: the
// generated file is excluded again with the same reason, the covered file is
// not re-judged, and the plain oversized file halts again.
func TestReviewRedriveSkipsTheSameFile(t *testing.T) {
	e := newEnv(t)
	writeRequiredHead(e, "a.go", "package a\n")
	big := oversizedHeaderBody()
	writeRequiredHead(e, "gen/big.ts", big)
	// A plain oversized file: ordinary short lines, so no built-in signal —
	// including minified — matches it.
	plain := strings.Repeat("bb\n", 70*1024)
	writeRequiredHead(e, "zz.go", plain)
	e.runner.script = []exec.Result{
		{ExitCode: 0, Stdout: claudeStdout(batchAnswer(t, 1))},
	}
	first := runLeg(t, e, e.request(t))
	...
	if first.Outcome != review.OutcomeHalted {
		t.Fatalf("first Outcome = %q, want halted on the plain oversized file", first.Outcome)
	}
	...
	gens := ledgerGenerations(t, e)[published:]
	if len(gens) == 0 {
	...
	for _, gen := range gens {
		for _, exclusion := range gen.Excluded {
			if exclusion.Path == "gen/big.ts" {
				found = true
				wantReason := "generated (header), " + strconv.Itoa(len(big)) + " bytes, over the 184320-byte prompt budget"
				if exclusion.Reason != wantReason {
					t.Errorf("re-drive exclusion reason = %q, want %q", exclusion.Reason, wantReason)
				}
			}
		}
	}
