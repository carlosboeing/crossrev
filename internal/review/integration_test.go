...
// The reported failure, replayed: a pull request shaped like the one that
// halted, with six ordinary files and one 350 KB generated file carrying a
...
func TestReviewSkipsTheGeneratedFileAndConverges(t *testing.T) {
	...
	answer := strings.Replace(batchAnswerFor(t, paths), `"verdict":"issues-remain"`, `"verdict":"converged"`, 1)
	e.runner.script = []exec.Result{
		{ExitCode: 0, Stdout: claudeStdout(answer)},
	}
	...
	if !containsString(e.forge.labelsAdded, policy.LabelConverged) {
		t.Errorf("labelsAdded = %v, want %q: a skip does not block convergence", e.forge.labelsAdded, policy.LabelConverged)
	}

	// The prompt packing measured never names the skip: the exclusion list
	// was rendered before packing found it, and the diff is sliced to the
	// batch's own files.
	if len(*prompts) != 1 {
	...
	for _, line := range got.Messages {
		if line.Kind == ui.KindWarn && strings.Contains(line.Text, "src/webAssets.ts") && strings.Contains(line.Text, "generated (header)") {
			warns++
		}
	}
	if warns != 1 {
		t.Errorf("skip warnings = %d, want 1", warns)
	}

	// The comment: warning block above the verdict, then the footnote.
	summary := e.forge.edits[len(e.forge.edits)-1]
	warning := strings.Index(summary, "**Warning: 1 changed file was not reviewed.**")
	if warning < 0 {
		t.Fatalf("no warning block:\n%s", summary)
	}
	if verdictAt := strings.Index(summary, "Verdict: **converged**"); verdictAt < 0 || warning > verdictAt {
		t.Errorf("the warning does not sit above the verdict:\n%s", summary)
	}
	for _, want := range []string{
		"generated (header: `// generated by scripts/build-embed.mjs — do not edit`)",
