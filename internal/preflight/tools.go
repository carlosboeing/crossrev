...
// installed is `command -v` over PATH (lib/preflight.sh:56). A nil LookPath is
// the shared search in internal/exec, which is bash's own contract: the first
// entry holding an executable regular file wins, and a directory named like the
// tool is not one.
