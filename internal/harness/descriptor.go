...
func Load(raw []byte) (Document, error) {
	if problem := Validate(raw); problem != "" {
		return Document{}, fmt.Errorf("%w: %s\n   It drives sourced paths, install commands, environment names and quarantine paths, so CrossRev stops rather than acting on it. Fix assets/harnesses.json.", ErrDescriptor, problem)
	}
